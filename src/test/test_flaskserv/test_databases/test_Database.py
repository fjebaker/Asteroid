...
class TestDatabase():
	def test_insert_row(self):
		with self.db(self.keys):
			self.db.insert_entire_row("songs",
					("test-name1", "test-artist1", 1, "test-path1", "test-metadata")
				)
			self.db.insert_entire_row("songs",
					("test-name2", "test-artist2", 2, "test-path2", "test-metadata")
				)
	def test_select_rows(self):
		...
		assert rows1 == (
					{'rowid': 1, 'name': 'test-name1', 'artist': 'test-artist1', 'duration': 1.0, 'file_path': 'test-path1', 'meta_dat': 'test-metadata'},
					{'rowid': 2, 'name': 'test-name2', 'artist': 'test-artist2', 'duration': 2.0, 'file_path': 'test-path2', 'meta_dat': 'test-metadata'}
				)
		assert rows2 == ({'rowid': 1, 'name': 'test-name1', 'artist': 'test-artist1', 'duration': 1.0, 'file_path': 'test-path1', 'meta_dat': 'test-metadata'},)
		assert rows3 == ({'rowid': 1, 'name': 'test-name1'}, {'rowid': 2, 'name': 'test-name2'})
	...
	def test_select_modifiers(self):
		...
		assert rows1 == ({'name': 'test-name1', 'artist': 'test-artist1'}, {'name': 'test-name2', 'artist': 'test-artist2'})
	...
	def test_latest_items(self):
		...
		assert rows1 == ({'rowid': 2, 'name': 'test-name2', 'artist': 'test-artist2', 'duration': 2.0, 'file_path': 'test-path2', 'meta_dat': 'test-metadata'},)

	def test_update(self):
		with self.db(self.keys):
			self.db.update_generic("songs", {5:"updated"}, {0:1})
			rows1 = self.db.get_all_rows("songs")
		assert rows1 == ({'rowid': 1, 'name': 'test-name1', 'artist': 'test-artist1', 'duration': 1.0, 'file_path': 'test-path1', 'meta_dat': 'updated'}, {'rowid': 2, 'name': 'test-name2', 'artist': 'test-artist2', 'duration': 2.0, 'file_path': 'test-path2', 'meta_dat': 'test-metadata'})
